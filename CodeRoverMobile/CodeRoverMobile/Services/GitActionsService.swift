...
// Depends on: CodeRoverService, GitActionModels
...
final class GitActionsService {
    ...
    init(coderover: CodeRoverService, workingDirectory: String?) {
    ...
    private func request(method: String, params: [String: JSONValue] = [:]) async throws -> [String: JSONValue] {
        ...
        do {
            let response = try await coderover.sendRequest(method: method, params: rpcParams)
    ...
    private func rememberRepoRoot(from result: GitRepoSyncResult?) {
        coderover.rememberRepoRoot(result?.repoRoot, forWorkingDirectory: workingDirectory)
    }
}
