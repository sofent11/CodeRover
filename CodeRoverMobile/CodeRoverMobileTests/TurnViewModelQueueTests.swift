...
final class TurnViewModelQueueTests: XCTestCase {
    ...
    func testSendTurnQueuesDraftWhenThreadBusy() {
        ...
        viewModel.composerMentionedFiles = [
            TurnComposerMentionedFile(
                ...
                path: "CodeRoverMobile/Views/Turn/TurnView.swift"
            )
        ]
        viewModel.composerMentionedSkills = [
            TurnComposerMentionedSkill(
                name: "check-code",
                path: "/Users/emanueledipietro/.coderover/skills/check-code/SKILL.md",
        ...
        viewModel.sendTurn(coderover: service, threadID: "thread-queue")

        XCTAssertEqual(viewModel.queuedCount(coderover: service, threadID: "thread-queue"), 1)
        ...
        XCTAssertFalse(viewModel.isQueuePaused(coderover: service, threadID: "thread-queue"))
        ...
        XCTAssertEqual(queued.text, "Please inspect @CodeRoverMobile/Views/Turn/TurnView.swift")
    ...
    func testFlushQueueDoesNothingWhenDisconnected() {
        ...
        viewModel.flushQueueIfPossible(coderover: service, threadID: "thread-queue")
    ...
    func testFlushQueueFailureRequeuesAndPausesQueue() async {
        ...
        XCTAssertEqual(viewModel.queuedCount(coderover: service, threadID: "thread-queue"), 1)
        XCTAssertTrue(viewModel.isQueuePaused(coderover: service, threadID: "thread-queue"))
        XCTAssertNotNil(viewModel.queuePauseMessage(coderover: service, threadID: "thread-queue"))
    ...
    func testResumeQueueClearsPauseState() {
        ...
        viewModel.resumeQueueAndFlushIfPossible(coderover: service, threadID: "thread-queue")
    ...
    func testQueuedDraftsPersistAcrossViewModelRecreationForSameThread() {
        ...
        firstViewModel.sendTurn(coderover: service, threadID: "thread-queue")

        let secondViewModel = makeViewModel()
        XCTAssertEqual(secondViewModel.queuedCount(coderover: service, threadID: "thread-queue"), 1)
        XCTAssertEqual(secondViewModel.queuedCount(coderover: service, threadID: "other-thread"), 0)
    }
    ...
    func testSendTurnStartsImmediatelyWhenRunningFlagRefreshClearsStaleBusyState() async {
        ...
        XCTAssertEqual(viewModel.queuedCount(coderover: service, threadID: "thread-queue"), 0)
    ...
    func testSteerQueuedDraftRemovesOnlySelectedRowAndPreservesOrder() async {
        ...
        viewModel.steerQueuedDraft(id: second.id, coderover: service, threadID: "thread-queue")
    ...
    func testSteerQueuedDraftKeepsFullPayloadShape() async {
        ...
        viewModel.steerQueuedDraft(id: draft.id, coderover: service, threadID: "thread-queue")
        await waitForSteerCompletion(viewModel)
    ...
    func testSteerQueuedDraftFailureKeepsRowAndDoesNotPauseQueue() async {
        ...
        service.requestTransportOverride = { _, _ in
            throw CodeRoverServiceError.rpcError(RPCError(code: -32000, message: "turn already completed"))
        }
        ...
        service.requestTransportOverride = { method, params in
            if method == "turn/steer" {
                ...
                if steerAttemptTurnIDs.count == 1 {
                    throw CodeRoverServiceError.rpcError(RPCError(code: -32000, message: "no active turn"))
                }
