...
Android client for the local-first Remodex bridge, implemented with Kotlin and Jetpack Compose.
...
- Local bridge connection skeleton with the Remodex secure handshake (`clientHello` -> `serverHello` -> `clientAuth` -> `secureReady` -> `resumeState`).
...
Open this folder as a standalone Android Studio project (`RemodexAndroid/`).
