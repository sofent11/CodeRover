...
const { HANDSHAKE_MODE_QR_BOOTSTRAP, HANDSHAKE_MODE_TRUSTED_RECONNECT, createBridgeSecureTransport, nonceForDirection, } = require("../src/secure-transport");
...
test("secure transport round-trips encrypted payloads after a trusted reconnect handshake", () => {
    ...
    const phoneSignature = sign(null, phoneAuthTranscript, createPrivateKey({
        key: {
            crv: "Ed25519",
    ...
    secureTransport.queueOutboundApplicationMessage(JSON.stringify({ id: "response-1", result: { ok: true } }), (message) => {
        wireMessages.push(message);
    });
...
test("qr bootstrap keeps previously paired phones trusted when a second phone scans", () => {
    ...
    secureTransport.handleIncomingWireMessage(JSON.stringify({
        ...
        phoneDeviceId: "phone-3b",
        keyEpoch: serverHello.keyEpoch,
        phoneSignature: sign(null, Buffer.concat([
...
test("qr bootstrap starts a fresh replay window instead of leaking buffered messages", () => {
    ...
    secureTransport.queueOutboundApplicationMessage(JSON.stringify({ id: "stale-response", result: { ok: true } }), (message) => {
        wireMessages.push(message);
    });
    assert.equal(wireMessages.length, 1);
    finishHandshake({
