...
function createCodexAdapterFixture({ threads = [], } = {}) {
...
function buildCodexThread({ threadId = "codex-thread-1", messageCount = 180, turnId = "turn-1", } = {}) {
...
function buildCodexHistoryResult(thread, history) {
    ...
    if (mode === "tail") {
        ...
        const anchorIndex = items.findIndex((item) => ((anchorItemId && item.id === anchorItemId)
...
function createDefaultCodexTransportFixture(manager, { threadRef, } = {}) {
    const readCountsByThread = new Map();
    const sentMethods = [];
    const transport = {
        send(message) {
...
(0, node_test_1.test)("Codex overlays keep local naming while refreshing upstream preview and updatedAt", async () => {
    const threadRef = {
        current: buildCodexThread({
    ...
    try {
        ...
        node_assert_1.strict.ok(initialResponse);
        node_assert_1.strict.equal(initialResponse.result.thread.preview, "message-2");
        ...
        node_assert_1.strict.ok(renameResponse);
        node_assert_1.strict.equal(renameResponse.result.thread.name, "Pinned Codex Name");
        threadRef.current = buildCodexThread({
            threadId: "codex-overlay-thread",
            messageCount: 3,
            turnId: "turn-overlay",
        ...
        node_assert_1.strict.ok(refreshedResponse);
        node_assert_1.strict.equal(refreshedResponse.result.thread.name, "Pinned Codex Name");
        node_assert_1.strict.equal(refreshedResponse.result.thread.preview, "message-3");
        node_assert_1.strict.equal(refreshedResponse.result.thread.updatedAt, threadRef.current.updatedAt);
        ...
        node_assert_1.strict.ok(listResponse);
        node_assert_1.strict.equal(listResponse.result.items[0].name, "Pinned Codex Name");
        node_assert_1.strict.equal(listResponse.result.items[0].preview, "message-3");
        node_assert_1.strict.equal(listResponse.result.items[0].updatedAt, threadRef.current.updatedAt);
    }
    finally {
        fixture.cleanup();
    }
