-- FILE: coderover-refresh.applescript
-- Purpose: Forces a non-destructive route bounce inside CodeRover so the target thread remounts without killing runs.
...
on run argv
  ...
  set targetUrl to ""
  set bounceUrl to "coderover://settings"
  ...
  my openCodeRoverUrl(bundleId, appPath, bounceUrl)
  delay 0.18

  if targetUrl is not "" then
    my openCodeRoverUrl(bundleId, appPath, targetUrl)
  else
    my openCodeRoverUrl(bundleId, appPath, "")
  end if
...
on openCodeRoverUrl(bundleId, appPath, targetUrl)
  try
    if targetUrl is not "" then
