...
const BIN_PATH = path.join(import.meta.dir, "..", "bin", "coderover.js");
...
test("CLI shim loads dist output and preserves command surface", () => {
  ...
  assert.match(source, /coderover up \| coderover resume \| coderover watch \[sessionId\]/);
});
...
test("CLI shim still executes through dist without a source fallback", () => {
  ...
  assert.match(result.stderr, /coderover up \| coderover resume \| coderover watch \[sessionId\]/);
});
